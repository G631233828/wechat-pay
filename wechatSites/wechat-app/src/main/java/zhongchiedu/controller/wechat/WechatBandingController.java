...
public class WechatBandingController {
	...
	public ModelAndView index(HttpSession session, HttpServletRequest request) {
		...
		// ModelAndView model = new ModelAndView();
		// WeChatbanding we =
		// this.weChatbandingService.findWeChatbandingByOpenId("ooiMKv1-3Ibrcw1Y9GV6CagesV0g");
		// if(Common.isEmpty(we)){
		// model.setViewName("redirect:toAuthor");
		// return model;
		// }
		// model.addObject("wechatbinding", we);
		// model.addObject("clazz",we.getStudentAccount().substring(0,
		// 4)+"年"+we.getStudentAccount().substring(5,6)+"班");
		// model.setViewName("wechat/front/index");
		return model;
	}
	...
	public BasicDataResult toBinding(String account, String password, String code, HttpServletRequest request) {
		...
		if (Common.isNotEmpty(account) && Common.isNotEmpty(password) && Common.isNotEmpty(code)) {
			...
			if (result.equals("error")) {
				...
				// 如果班级不存在则添加班级
				String clazzYear = account.substring(0, 4);
				String clazzNum = account.substring(4, 6);
				// 通过班级年份班级号查询班级如果不存在则创建班级
	...
	//
	// List<WeChatbanding> list = this.weChatbandingService.find(new Query(),
	// WeChatbanding.class);
	//
